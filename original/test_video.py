...
ap.add_argument("-video","--videoPath", type = str, 
    help = "path to video")
ap.add_argument("-videoEmbedding","--videoPathEmbedding", default = "examples/fine_tuning/Pedro.mp4", type = str, 
...
path_to_model = 'model_weights.tar'
...
def embedder_inference():
	...
	frame_mark_video = generate_cropped_landmarks(frame_mark_video, pad=padding)
	print("Transforming...")
	frame_mark_video = torch.from_numpy(np.array(frame_mark_video)).type(dtype = torch.float) #T,2,256,256,3
	frame_mark_video = frame_mark_video.transpose(2,4).to(device) #torch.Size([32, 2, 3, 256, 256])
...
def finetuning_training():
	...
	if not display_training:
	    matplotlib.use('agg')
	dataset = FineTuningVideoDataset(path_to_embedding_video, device, padding)
